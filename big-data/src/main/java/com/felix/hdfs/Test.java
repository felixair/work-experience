...
public class Test {
	...
	static String filePath = "C:/local_data/upload/test.csv";
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (item.length >= 2) {
					...
					String others = "";
					if (item.length > 2) {
						others = line.substring(item[0].length() + item[1].length() + 2);
					}
	 ...
	 * @desc 保存CSV文件
	 */
	public static void saveCsvFile(List<Map<String, String>> csvList, String fileName) {
